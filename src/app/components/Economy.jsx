...
export default function Economy() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const data = await getNewsByCategory("economy", "en");
  ...
  if (error) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-10 text-[#57832A] text-center">
        {error}
      </div>
    );
  }
          ...
          {topFeatured.map((news) => (
            <Link key={news?._id} href={`/news/${news?._id || "#"}`}>
              <article className="group relative overflow-hidden bg-white border border-gray-200 hover:shadow-lg transition">
                ...
                />
                <div className="p-5">
                  <span className="bg-[#57832A] text-white text-[10px] px-2 py-1 uppercase inline-block mb-2">
                    {getTranslatedValue(news?.category, lang)}
                  </span>
                  <h3 className="text-xl font-bold line-clamp-2 group-hover:text-[#57832A] transition mb-2">
            ...
            <Link key={news?._id} href={`/news/${news?._id || "#"}`}>
              <article className="group bg-white border border-gray-200 overflow-hidden hover:shadow-lg transition h-[380px] flex flex-col">
                ...
                <div className="p-4 flex-grow flex flex-col">
                  <h3 className="text-base font-semibold group-hover:text-[#57832A] transition mb-2 line-clamp-2">
                    {getTranslatedValue(news?.title, lang)}
                  </h3>
